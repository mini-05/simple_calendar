...
void main() {
  group('RecurrenceRule - OCP 기반 반복 일정 확장 (expand)', () {
    ...
    // 💡 [개선] Dart 3.0 Records 적용: limit 대신 from, to를 사용하여 Windowing 검증
    final recurrenceCases = [
      (
        ...
        from: DateTime(currentYear, 1, 1),
        to: DateTime(currentYear, 12, 31),
        expectedLength: daysInYear,
        expectedLast: DateTime(currentYear, 12, 31),
      ),
      (
        description:
            '매일(Daily) 반복 시 200년 뒤를 종료일로 잡고 전체 범위를 조회하면, 안전장치(하드리밋)에 의해 정확히 5000개만 생성되어야 한다',
        start: start200Years,
        rule: RecurrenceRule(
          frequency: RecurrenceFrequency.daily,
          interval: 1,
        ...
        to: end200Years,
        expectedLength: 5000,
        expectedLast: start200Years.add(const Duration(days: 4999)),
      ),
      (
        description:
            '종료일이 200년 뒤라도 윈도우(to)를 당일로 제한하면, 정확히 시작일 하루(1개)만 생성되어야 한다',
        start: start200Years,
        rule: RecurrenceRule(
          frequency: RecurrenceFrequency.daily,
          interval: 1,
      ...
      (
        description: '종료일이 1년 뒤라도 윈도우(to)를 3일치로 제한하면, 정확히 3일 치만 생성하고 멈춰야 한다',
        start: DateTime(currentYear, 1, 1),
        rule: RecurrenceRule(
          frequency: RecurrenceFrequency.daily,
          interval: 1,
    ...
    for (final tc in recurrenceCases) {
      test(tc.description, () {
        // 💡 [수정됨] limit 대신 required 파라미터인 from, to 주입
        final result = tc.rule.expand(tc.start, from: tc.from, to: tc.to);
  ...
  group('CalendarEvent - OCP 기반 직렬화/역직렬화 10종 완벽 검증', () {
    ...
    for (var i = 0; i < eventCases.length; i++) {
      test('CalendarEvent 케이스 ${i + 1} 변환 무결성 검증', () {
        ...
        expect(
          CalendarEvent.fromJson(original.toJson()).toJson(),
          original.toJson(),
        );
      });
    }
