...
5. **파일 헤더 규칙:** 모든 dart 파일 1번째 줄은 `// v실제버전` (예: `// v4.5.8`), 2번째 줄은 작성 AI에 따라 `// claude_파일명.dart` 또는 `// gemini_파일명.dart`, 3번째 줄은 `// lib/경로/파일명.dart`. pubspec.yaml도 1번째 줄에 `# v실제버전` 주석 표기, `version:` 필드는 `x.x.x+y` 형식 유지. (**단, 내용이 실제로 수정될 때만 버전을 갱신한다.**)
...
* **v4.5.6:**
  * **OCP 기반 달력 네비게이션 통일:** '오늘' 버튼 또는 검색을 통한 날짜 이동 시 스와이프/화살표 뷰 모드에 상관없이 애니메이션 없이 즉시 화면이 전환(`jumpToPage`)되도록 공통 로직 적용 및 불필요한 분기문 제거.
  * **음력 표시 버그 수정:** 음력 설정 On 상태에서 일요일뿐만 아니라 모든 날짜 셀에 음력 표기가 노출되는 버그를 `day.weekday == DateTime.sunday` 조건 추가로 완벽 해결.
* **v4.5.7:**
  * **스플래시 플립 시각적 버그 1차 시도:** 숫자의 일부 글자색이 다르게 나타나는 현상을 수정하기 위해 모든 카드의 텍스트 컬러를 완전한 하얀색으로 변경.
* **v4.5.8 (현재 버전):**
  * **스플래시 플립 시각적 단절 및 겹침 버그 완벽 해결:** 애니메이션 시작 전후로 위아래 카드의 디자인(투명도 박스 여부)이 달라 숫자가 짝짝이로 끊겨보이던 치명적 결함을 완벽히 고침. `isBackground` 파라미터를 완전히 제거하여 모든 카드 조각을 100% 동일한 디자인(파란색 반투명 베이스 + 순백색 텍스트)으로 통일하고, 애니메이션이 정지된 상태(`val == 0.0` 또는 `val == 1.0`)일 때는 플립 조각을 렌더링에서 완전히 제외하여 카드가 두 겹으로 겹쳐서 어두워지는 현상을 완벽 차단.
